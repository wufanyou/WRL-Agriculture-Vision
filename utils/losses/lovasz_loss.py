...
__all__ = ["CustomizeLovaszLoss", "CustomizeLovaszLossV2"]
...
class CustomizeLovaszLoss(nn.Module):
    def __init__(self, **kwargs):
        super(CustomizeLovaszLoss, self).__init__()
        self.lovasz_loss = LovaszLoss(mode="multilabel", ignore_index=255)
    ...
    def forward(
        ...
        if mask is not None:
            ...
            target[mask == 0] = 255
        return self.lovasz_loss(pred, target)


class CustomizeLovaszLossV2(nn.Module):
    def __init__(self, **kwargs):
        super(CustomizeLovaszLossV2, self).__init__()
        self.lovasz_loss = LovaszLoss(mode="multiclass", ignore_index=255)

    def forward(self, pred: Tensor, target: Tensor, mask) -> Tensor:
        target = target.argmax(1)  # [N,W,H]
        target[mask == 0] = 255
        return self.lovasz_loss(pred, target)
