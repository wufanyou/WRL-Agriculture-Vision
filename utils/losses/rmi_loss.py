...
# _IS_SUM = False  # 	sum the loss per channel
#
#
...
#
#     def forward(self, logits_4D, labels_4D, mask):
#         # explicitly disable fp16 mode because torch.cholesky and
#         # torch.inverse aren't supported by half
#         logits_4D.float()
#         labels_4D.float()
#         loss = self.forward_sigmoid(logits_4D, labels_4D, mask)
#         return loss
#
#     def forward_sigmoid(self, pred, target, mask):
#
#         N, C, W, H = pred.shape
#         mask = mask[:, None].expand([N, C, W, H])
#         bce_loss = F.binary_cross_entropy_with_logits(pred, target, mask)
#         rmi_loss = self.rmi_lower_bound(pred, target)
#
#         # add together
...
#
#         la_vectors = la_vectors.view([n, c, self.half_d, -1]).double()#.requires_grad_(False)
#         pr_vectors = pr_vectors.view([n, c, self.half_d, -1]).double()
#
