...
class Conv3x3GNSiLU(nn.Module):
    def __init__(self, in_channels, out_channels, upsample=False):
        ...
        if self.upsample:
            self.interpolate = Upsample(out_channels)
...
class Upsample(nn.Sequential):
    def __init__(self, channels):
        ...
        seq.append(nn.GroupNorm(channels, channels))
        seq.append(nn.SiLU(inplace=True))
        super(Upsample, self).__init__(*seq)
...
class FPNBlock(nn.Module):
    def __init__(self, pyramid_channels, skip_channels):
        ...
        self.interpolate = Upsample(pyramid_channels)
